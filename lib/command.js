...
module.exports = class Command {
  ...
  async forbidden(msg, event) {
    ...
    if (ev.admin && !msg.sender.admin && (!msg.key.fromMe && !this.config.ownerNumber.find((a) => msg.sender.jid.includes(a)))) return this.action(ev.admin, 'admin', msg)
...
// this.client.chats[msg.from].antispam++;
// setTimeout(() => {
//     this.client.chats[msg.from].antispam--;
//     },4000);
    // if (!msg.from.endsWith('@g.us')) {
    // if (this.client.chats[msg.from].antispam >= 3) return console.log(msg.sender.jid.split('@')[0]+` Terdeteksi Spam!, Tunggu beberapa saat lagi...`)
    // // this.action(`Terdeteksi Spam!, Tunggu beberapa saat lagi...`,"", msg)
    // }
    return 200;
  }
  async execute(msg) {
