...
_*Pencarian Ditemukan*_\n\n`
            ...
            await client.sendAdReply(msg.from, { text: captions + `*MechaBot*` }, { title: data.title, description: config.botname, matchedText: ytsr[0].url, canonicalUrl: global.linkgcme, thumbnail: ytsr[0].thumbnail, previewType: 2, renderLargerThumbnail: true, mediaType: 1 })
        } catch (err) {
  ...
  Data Berhasil Didapatkan:\n`
    ...
    if (command == "play" || command == "lagu" || command == "song") {
        try {
            ...
            //     // await client.sendAdReply(msg, { audio: done }, { title: res.result.title, body: as[0].size, thumbnail: as[0].thumb, meidaType: 1, meidaUrl: res[0].url, soutceUrl: res[0].url })
            let data = await await ytdl2(sul, "mp3", "128")
            //   captions =
            //     `*_${head}${line.repeat(4)}${list} Youtube Downloader_*
            // Data Berhasil Didapatkan:\n`
