...
module.exports = class Functions {
  ...
  createExif(packname, authorname, filename) {
    ...
    let json = {
      'sticker-pack-id': 'Ikyy Ads',
      'sticker-pack-name': "@Ikiyyye",
      'sticker-pack-publisher': "Kurosaki-Bot",
    };
  ...
  async run() {
    try {
      ...
      for (let a of fs.readdirSync("./src/json/")) {
        if (!a.includes('.json')) continue;
        database[a.replace(".json", '')] = require(`../src/json/${a}`);
      }
      ...
      global.client = new (require('./waconnection.js'))(baileys.default({
        ...
        browser: ['Mecha-Bot', 'Desktop', '3.0'],
        logger: require('pino')({
          level: 'silent'
        })
      }));
      if (Object.keys(client.chats).length == 0) client.chats = database.chats || {};
